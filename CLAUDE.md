...
- **Deploy sandbox trên server này (không qua git/GitHub Action):** repo `qor_manage_all` sống trên cùng host với `/var/www/dcr` (không phải máy dev riêng), nên khi cần đưa code sandbox đang sửa lên chạy thật, dùng `./deploy-sandbox.sh [app]` ở root repo — script build local (không cần commit/push) rồi copy thẳng artifact vào `/var/www/dcr/sandbox/<app>` + `pm2 reload <app>-sandbox`. Script tự kiểm tra branch hiện tại của từng submodule, **chỉ deploy app nào đang ở branch `sandbox`** (skip nếu không phải). Chỉ đụng `/var/www/dcr/sandbox/**`, không bao giờ đụng `/var/www/dcr/<app>` (prod).
  - Đây là quy trình **khác hoàn toàn** với GitHub Actions `deploy-sandbox.yml` của từng submodule (trigger khi `git push` nhánh `sandbox` lên GitHub, chạy trên CI qua SSH) — hai quy trình độc lập, không phụ thuộc nhau. `deploy-sandbox.sh` là đường tắt local để test nhanh trên chính server, không thay thế cho việc commit/push khi user muốn CI deploy chính thức.
  - Vì đây là deploy thật (npm install, `prisma migrate deploy` cho api, restart PM2 sandbox) — **hỏi user trước khi chạy** trừ khi user đã yêu cầu rõ ràng.
